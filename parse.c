...
char *parse_oper_strings[] = {"PLUS", "MINUS", "MULT", "DIV", "NOT", "LSL", "LSR", "AND", "OR", "XOR", "ASR"};
...
struct parse_node_st * parse_expression(struct parse_table_st *pt, 
    ...
    while (true) {    
        ...
        if (tp->id == TK_PLUS || tp->id == TK_MINUS
            ...
            || tp->id == TK_LSL || tp->id == TK_LSR || tp->id == TK_AND || tp->id == TK_OR || tp->id == TK_XOR || tp->id == TK_ASR) {
...
struct parse_node_st * parse_operand(struct parse_table_st *pt,
    ...
    if (scan_table_accept(st, TK_INTLIT)) {
        ...
        np1->type = EX_INTVAL;
        np1->intval.value = conv_decstr_to_uint32(tp->value);
        ...
        np1 = parse_node_new(pt);
        np1->type = EX_INTVAL;
        np1->intval.value = conv_hexstr_to_uint32(tp->value);
